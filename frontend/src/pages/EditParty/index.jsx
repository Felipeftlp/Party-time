...
import usToast from "../../hooks/useToast";
  ...
  const updateParty = async (e) => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        usToast(res.data.msg);
      }
    } catch (err) {
      usToast(err.response.data.msg, "error");
    }
  };
      ...
      <form onSubmit={(e) => updateParty(e)}>
        ...
        <label>
          ...
          <input
            ...
            onChange={(e) => setParty({ ...party, image_url: e.target.value })}
            value={party.image_url}
            required
          />
        </label>
        <div>
